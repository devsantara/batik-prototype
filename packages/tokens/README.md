# `@batik-prototype/tokens`

Fluid type and space scales, built on [`@batik-prototype/math`](../math).

```ts
import { fluid, space } from '@batik-prototype/tokens';
...
- **`@batik-prototype/math` is a `dependency`, declared `workspace:*`.** `pnpm publish`
...
- **Cross-package imports resolve to source, so no build is needed to work.**
  `@batik-prototype/math`'s top-level `exports` points at its `src/index.ts`, so a fresh
  clone type-checks and tests green with no `dist`, and editing math is visible
  here immediately. See [`@batik-prototype/math`](../math#why-this-package-looks-the-way-it-does)
  for how the published shape is kept separate.
- **The `build` task declares that dependency for ordering.** `dependsOn: [{
task: 'build', from: 'dependencies' }]` means `vp run @batik-prototype/tokens#build`
  ...
  resolve. It lives in [`@batik-prototype/config`](../config#readme) with the
...
- **Everything else** — the tsconfig presets, the `#/*` imports map, the pack
  gates — matches [`@batik-prototype/math`](../math#why-this-package-looks-the-way-it-does).
