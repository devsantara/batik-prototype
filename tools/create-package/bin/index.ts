...
import { PACKAGES_DIRECTORY, findWorkspaceRoot } from '../src/workspace.ts';
...
// `vp create package -- --name color` would otherwise scaffold into tools/.
// Derive the destination from the package name instead, anchored at the
// workspace root rather than the cwd, and leave an explicit --directory alone.
//
...
if (values.directory === undefined && typeof values.name === 'string') {
  const target = path.join(findWorkspaceRoot(), PACKAGES_DIRECTORY, values.name);
